...
use think\Db;
use think\log;
class checkTestOrder extends Command
{
    protected function configure()
    {
        parent::configure(); // TODO: Change the autogenerated stub
        $this->setName('doTestOrder')->setDescription('处理试玩库遗漏注单');
    }
    ...
    protected function  execute(Input $input, Output $output)
    {
        ...
        while(1){
            ...
            $list = $db->query("select * from ffc_order_missing where o_is_deal=0 limit 1000");//一次处理一千条
            echo "注单处理开始\n";
            if(empty($list)){
                echo "暂无需要处理的遗漏注单\n";
                break;
            ...
            foreach ($list as $k=>$v){
                $arr = [
                    'user'=> substr($v['o_username'],strpos($v['o_username'],'_')+1),
                    'siteId'=> $v['o_siteId'],
                    'transId' => $v['o_transId']
                ];
                ...
                if($res == 1){
                    try{
                        ...
                        $sendData['username'] = substr($v['o_username'],strpos($v['o_username'],'_')+1);
                        $sendData['orders'] = [$v];
