 ...
 * 获取param
 */
function getParam($str5,$public_key){
    $pu_key = openssl_pkey_get_public($public_key);//这个函数可用来判断公钥是否是可用的
    $param1 = public_encrypt($str5,$pu_key);
    return  $param1;
}
...
<div style=" width:100%; text-align:center; margin:20px auto auto; overflow:hidden;">
    ...
    <!--param RSA加密-->
    <form action="http://pc.newgfc.com" method="post">
        <label>param:<input type="text" name="param" readonly value="<?php echo $param1;  ?>"/></label>
        <label>encrypt:<input type="text" name="encrypt" readonly value="<?php echo $sha; ?>"/></label>
        <label>clientType:
        ...
        <label>lid:<input type="text" name="lid" value="2"/></label>
