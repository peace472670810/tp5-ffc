(function(host, undefined) {
	var defConfig = {
			...
			successTip : ['<div class="bd text-center">',
								'<div class="pop-title">',
									...
									'<h4 class="pop-text"><#=msg#><br /></h4>',
								'</div>',
								'<p class="text-note">您可以通过底部”<#=gameType#>“查询您的投注记录！</p>',
							'</div>'].join(''),
			//提醒选求提示
	...
	var pros = {
		...
		init: function(cfg){
			...
			me.win = new phoenix.MiniWindow({
				//实例化时追加的最外层样式名
				cls:'pop w-9',
				confirmButtonText:'btn-important',
                cancelButtonText:'ui-js-btn-cancel',
                closeButtonText:'ui-js-btn-closetip'
			});
			me.mask = new phoenix.Mask.getInstance();
			//绑定隐藏完成事件
			me.reSet();
