	...
	var pros = {
		...
		initEvent: function() {
			...
			setTimeout(function() {
				...
				if ($.trim(nowGameType) != 'slmmc') { //秒秒彩无此结构，不能进入
					me.buildStartNumberSelectDom();
				}
		...
		trace_advanced: function() {
			...
			//盈利/盈利率追号不支持混投
			if (advancedType == 'fanbei') {} else if (!me.isSameGameMethod() && (advancedType == 'yingli' || advancedType == 'yinglilv') || isMoreBouns != '') {
		...
		getPlayGroupMoneyByGameMethodName: function(name) {
			var groupMoney = Games.getCurrentGame().getDynamicConfig()['gamelimit'][name]['usergroupmoney'],
